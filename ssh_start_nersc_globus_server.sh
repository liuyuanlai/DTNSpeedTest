...
do
	...
	ssh -f dtn$nid "globus-gridftp-server -aa -p $from_port -rp $from_server_folder,$to_server_folder,/dev/zero,/dev/null &"
	#ssh -f dtn$nid "globus-gridftp-server -aa -p $to_port -rp /dev &"
done
