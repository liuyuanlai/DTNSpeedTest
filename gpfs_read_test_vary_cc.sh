...
filePath=${gpfs}gf_test/read_test_results/vary_cc_result/${fileTimeStamp}_posix.txt
touch $filePath
...
do
	...
	echo $cc
	echo $cc >> $filePath
	/usr/bin/time -a -o $filePath -f "%e" globus-url-copy -q -cc $cc -f ${gpfs}gf_test/l2g_test_files/h2hList.txt
	timeStamp=$(date "+%Y.%m.%d-%H.%M.%S")
	echo $timeStamp >> $filePath
done	
