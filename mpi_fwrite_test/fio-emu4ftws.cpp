...
int main(int argc, char** argv) {
    ...
    char file_size[MAX_FNAME_LEN], log_buffer[MAX_LOG_LEN], file_name[MAX_FNAME_LEN];
    FILE* fp;
    ...
    if (rank == 0) {
        ...
        for(int r=1; r<np; r++){
            if (getline(&tmp_rdbuf, &wr_size, fp) != -1){ // use fscanf if ost information is provided
                tmp_rdbuf[strlen(tmp_rdbuf)-1] = '\0'; // remove CR
                strcpy(file_size, tmp_rdbuf);
                ret = MPI_Send(&file_size, MAX_FNAME_LEN, MPI_CHAR, r, 0, MPI_COMM_WORLD);
            }            
        }
        ...
        while (1){
            ...
            if (getline(&tmp_rdbuf, &wr_size, fp) != -1){ // use fscanf if ost information is provided
                ...
                strcpy(file_size, tmp_rdbuf);
                //printf("File size:%s\n", file_size);
                ret = MPI_Send(&file_size, MAX_FNAME_LEN, MPI_CHAR, req_rank, 0, MPI_COMM_WORLD);
            }
            else{ // when there is no more file to transfer
                file_size[0] = '\0'; // send an empty file name to represent finishing 
        ...
        ret = MPI_Recv(file_size, MAX_FNAME_LEN, MPI_CHAR, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        while(1){
            ...
            strcpy(file_name, "/global/cscratch1/sd/yuanlai/gf_test/write_tmp/write_test");
            sprintf(r, "%d", rank);
            strcat(file_name, r);
            ...
            f_size = strtol(file_size, NULL, 10);
            while(1){
                //printf("File size:%lld\n", f_size);
                if (f_size > BLOCK_SIZE) w_s = BLOCK_SIZE;
                else w_s = f_size;
                //printf("To write size:%lld\n", w_s);
                gettimeofday(&start, NULL);
                wr_size = fwrite(io_buffer, 1, w_s, fp);
                ...
                cfread ++;
                f_size -= BLOCK_SIZE;
                if (wr_size < BLOCK_SIZE || f_size == 0){
                    break;
                }
