...
filePath=${lustre}gf_test/new_read_test_results_all_ost/vary_cc_result/${fileTimeStamp}.txt
touch $filePath
...
do
	...
	echo $cc
	echo $cc >> $filePath
	/usr/bin/time -a -o $filePath -f "%e" globus-url-copy -q -cc $cc -f ${lustre}gf_test/new_test_files_all_ost/toNullList
	timeStamp=$(date "+%Y.%m.%d-%H.%M.%S")
	echo $timeStamp >> $filePath
done	
